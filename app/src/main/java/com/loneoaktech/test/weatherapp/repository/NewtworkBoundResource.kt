...
abstract class NetworkBoundResource<T>{
    ...
    @MainThread
    protected abstract fun onFechFailed()

    fun getAsLiveData(): LiveData<AsyncResource<T>> = _result


    private val _result = MediatorLiveData<AsyncResource<T>>()

    // Must be called by derived class constructor
    protected fun init() {
        _result.value = AsyncResource.loading()
    ...
    private fun fetchFromNetwork(dbLiveData: LiveData<T>){
        ...
        _result.addSource(dbLiveData, {newData -> _result.value = AsyncResource.loading(newData)})
